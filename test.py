...
class QuestView:
    def __init__(self, root, controller):
        ...
        # Add a progress bar to track quest completion
        self.progress_frame = ttk.Frame(self.left_frame)
        self.progress_frame.pack(fill='x', pady=5)

        self.progress_label = ttk.Label(self.progress_frame, text="Quests Completed: 0%", bootstyle="info")
    ...
    def update_progress(self):
        total_quests = len(self.controller.get_quests())
        if total_quests == 0:
            self.progress_bar["value"] = 0
            self.progress_label.config(text="Quests Completed: 0%")
            return

        completed_quests = sum(1 for quest in self.controller.get_quests() if quest.complete)
        progress = (completed_quests / total_quests) * 100
        self.progress_bar["value"] = progress
        self.progress_label.config(text=f"Quests Completed: {int(progress)}%")
...
if __name__ == "__main__":
    ...
    icon = PhotoImage(file="shaqqqIcon.png")
